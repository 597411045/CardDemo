...
public class CS_Card : NetworkBehaviour
{
    ...
    public void EndDrag()
    {
        ...
        foreach (var i in hits)
        {
            ...
            foreach (var j in i.gameObject.GetComponents<Component>())
            {
                ...
                {
                    CS_ClientPlayGameMode.localPlayer.GetComponent<CS_Player>().MoveACard(this.gameObject, j.gameObject);
    ...
    public void OnClick()
    {
        if (isOwned)
        {
            CS_ClientPlayGameMode.localPlayer.GetComponent<CS_Player>().ClickOnOwned(this.gameObject);
        }
        else
        {
            CS_ClientPlayGameMode.localPlayer.GetComponent<CS_Player>().ClickOnOther(this.gameObject);
        }

        CS_ClientPlayGameMode.localPlayer.GetComponent<CS_Player>().Cmd_AddOne();
    }
}